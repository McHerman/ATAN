...
class SysController(implicit c: Configuration) extends Module {
  ...
  val opbuffer   = Module(new BufferFIFO(256, new SysOP))
  // TODO Replace with sem equivalent.
  val readBuffer = Module(new BufferFIFO(256, new Bundle { val addrPkg = new addrPkg; val size = UInt(8.W); val rows = UInt(8.W) }))
