...
class LoadController(implicit c: Configuration) extends Module {
  ...
  switch(StateReg) {
    ...
    is(2.U) { // Stream AXIST data into DMA, wait for completion
      ReadDMA.io.interface.response.ready := true.B

      ReadDMA.io.dataIn.get.request.ready := io.AXIST.tvalid
      io.AXIST.tready := ReadDMA.io.dataIn.get.request.valid

      ReadDMA.io.dataIn.get.response.valid := io.AXIST.tvalid
      ReadDMA.io.dataIn.get.response.bits.readData := io.AXIST.tdata
