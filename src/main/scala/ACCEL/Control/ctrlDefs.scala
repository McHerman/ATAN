...
class SemProgRow extends Bundle {
  val depCount = UInt(log2Ceil(InstructionSet.MaxSemDeps + 1).W)
  val depAddrs = Vec(InstructionSet.MaxSemDeps, UInt(InstructionSet.SemDepAddrBits.W))
}
