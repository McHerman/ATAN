...
/**
 * Full-stack test for the compiler-generated riscv-16x16-coexecute program
 ...
 *   - test/riscv-16x16-coexecute.eaac (accelerator instructions, no preloads)
 *   - src/test/scala/ACCEL/MCC/riscv-16x16-coexecute.ll (two chained mcc
 *     kernels: relu on the i32 accumulator, then requantize to i8)
 *   - test/riscv-16x16-coexecute.reference.json (golden inputs/outputs)
 *
 * This is a genuine 16x16 matmul (arg0 . arg1 -> i32 accumulator, matching
 * the array's full native width -- the assembled Execute instruction's size
 * field is 16, exactly arrayDim, so there's no smaller-than-array lane
 * gating to worry about). mcc relu's and requantizes the accumulator down
 * to i8, handing the result back via a second semaphore for the
 * accelerator to stream out.
 *
 ...
 * below), which was confirmed to reproduce plain A @ B exactly by reading
 * back the raw pre-mcc accumulator (buf2) via hostIn.
 */
class Riscv16x16CoexecuteE2ETest extends AnyFreeSpec with Matchers with ChiselSim {
  ...
  "accelerator + mcc co-execute the compiler-generated 16x16-config program and match the reference output" in {
    val eaacPath = "/home/karlhk/dtu/Thesis/hardware/ATAN/test/riscv-16x16-coexecute.eaac"
    val hexPath  = "/home/karlhk/dtu/Thesis/hardware/ATAN/test/riscv-16x16-coexecute/riscv-16x16-coexecute.memhex"
    ...
    val jsonStr = Source.fromFile("/home/karlhk/dtu/Thesis/hardware/ATAN/test/riscv-16x16-coexecute.reference.json").mkString
    val parsed = for {
      json  <- parse(jsonStr)
    ...
    simulate(new AtanMcc16x16DUT(testConfig, memCfgBase = msCfg, mccInitData = initData)) { dut =>
      ...
      // arg0 (X/activation) is fed in natural row order; arg1 (Y/weight) is
      // fed row-reversed to match the systolic array's weight-stationary
      // shift-in order. Confirmed against the raw pre-mcc accumulator
      // (buf2), which then matches plain A @ B exactly with no output-side
      // reversal either.
